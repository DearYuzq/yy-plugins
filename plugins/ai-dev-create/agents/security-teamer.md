...
Light 模式必须执行以下检查，对应 `templates/security-standards.md` 中通用安全 12 项的关键子集。跳过其他深度分析：

| # | 检查项 | 攻击向量 | 必须输出的防御方案 | 标准关联 |
|---|--------|----------|-------------------|----------|
| L1 | SQL 注入 | `' OR '1'='1`; `; DROP TABLE` | 参数化查询 + 输入验证 | 通用 #2 |
| L2 | XSS | `<script>alert(1)</script>` | 输出编码 + CSP | 通用 #3 |
| L3 | 认证绕过 | 伪造 Token、暴力破解 | JWT 验证 + 限速 | 通用 #4 |
| L4 | 越权访问 | IDOR 水平/垂直越权 | 权限检查中间件 | 通用 #4 |
| L5 | 硬编码凭据 | 密钥/密码明文 | 环境变量/密钥管理 | 通用 #5 |

Light 模式完成标准：以上 5 项检查全部完成，CRITICAL 漏洞有对应防御设计。不做深入的业务逻辑攻击分析或竞态条件分析。

### Full 模式检查清单（/sdd-full）

完整检查所有 8 个攻击向量（通用安全标准 12 项 + 攻击向量清单 4 类）：
...
Full 模式使用 8 类攻击向量（详见上方 Full 模式检查清单）。每类攻击按以下 4 种手法执行:
...
4. **优先级评分（5 分制）**:
   ...
   - **严重程度** (确定可利用=4, 较可能=3, 需特定条件=2, 理论可能=1)
   - **攻击概率** (确定可利用=4, 较可能=3, 需特定条件=2, 理论可能=1)
...
在执行任何安全扫描命令前，必须先检查工具是否可用：

1. **npm audit**: 检查 `package.json` 是否存在 → 存在则可用
2. **semgrep**: 运行 `which semgrep` → 不存在则降级为"手动规则检查"
3. **pip-audit**: 运行 `which pip-audit` → 不存在则降级为"依赖版本 CVE 手动查询"
4. **safety**: 运行 `which safety` → 不存在则跳过
5. **OWASP dependency-check**: 运行 `which mvn` → 不存在则跳过

**降级策略**:
- 工具不存在: 标记为 ⚠️ TOOLS_MISSING，改用基于代码模式的静态检查
- 部分工具可用: 只运行可用的工具，对不可用的工具在报告中标明 "⚠️ {工具名} 未安装，使用替代检查"
- 所有工具均不可用: 降级为人工代码审查模式，手动检查 OWASP Top 10 对应的代码模式
...
|------|------|----------|
| npm audit | ✅ 可用 | - |
| semgrep | ⚠️ 未安装 | 手动规则检查 |
| pip-audit | ✅ 可用 | - |
```

### 依赖漏洞扫描

根据工具检查结果，运行项目语言对应的安全扫描（**仅运行已确认可用的工具**）：
...
Security Teamer Agent 的唯一职责是产出 `07-security-report.md`。**不再负责生成收敛摘要（convergent-summary.md）**。该职责已移至 Orchestrator，由 Orchestrator 读取 05/06/07 报告后自行生成。
