...
| 工具                     | 描述                                                                                                                                                                                | 需要权限 |
| :--------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--- |
| `Agent`                | 生成一个具有自己 context window 的 [subagent](/zh-CN/sub-agents)，用于处理任务                                                                                                                    | 否    |
| `AskUserQuestion`      | 提出多选问题以收集需求或澄清歧义                                                                                                                                                                  | 否    |
| `Bash`                 | 在您的环境中执行 shell 命令。请参阅 [Bash 工具行为](#bash-tool-behavior)                                                                                                                            | 是    |
| `CronCreate`           | 在当前会话中安排定期或一次性提示（Claude 退出时消失）。请参阅[计划任务](/zh-CN/scheduled-tasks)                                                                                                                  | 否    |
| `CronDelete`           | 按 ID 取消计划任务                                                                                                                                                                       | 否    |
| `CronList`             | 列出会话中的所有计划任务                                                                                                                                                                      | 否    |
| `Edit`                 | 对特定文件进行有针对性的编辑                                                                                                                                                                    | 是    |
| `EnterPlanMode`        | 切换到 Plan Mode 以在编码前设计方法                                                                                                                                                           | 否    |
| `EnterWorktree`        | 创建一个隔离的 [git worktree](/zh-CN/common-workflows#run-parallel-claude-code-sessions-with-git-worktrees) 并切换到它。传递 `path` 以切换到当前存储库的现有 worktree，而不是创建新的                                | 否    |
...
| `PowerShell`           | 在 Windows 上执行 PowerShell 命令。选择加入预览。请参阅 [PowerShell 工具](#powershell-tool)                                                                                                          | 是    |
| `Read`                 | 读取文件内容                                                                                                                                                                            | 否    |
...
权限规则可以使用 `/permissions` 或在[权限设置](/zh-CN/settings#available-settings)中配置。另请参阅[工具特定权限规则](/zh-CN/permissions#tool-specific-permission-rules)。
...
在 Windows 上，Claude Code 可以本地运行 PowerShell 命令，而不是通过 Git Bash 路由。这是一个选择加入的预览。
...
Claude Code 自动检测 `pwsh.exe`（PowerShell 7+），回退到 `powershell.exe`（PowerShell 5.1）。Bash 工具与 PowerShell 工具一起注册，因此您可能需要要求 Claude 使用 PowerShell。
...
* Auto mode 目前不适用于 PowerShell 工具
* PowerShell 配置文件未加载
* 不支持 sandboxing
* 仅在本机 Windows 上支持，不支持 WSL
* 仍需要 Git Bash 来启动 Claude Code
